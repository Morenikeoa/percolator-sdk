...
export function encodeUnwrapEscrowedPortfolio(args: UnwrapEscrowedPortfolioArgs): Uint8Array {
  const data = concatBytes(
 ...
 */
export const INIT_MATCHER_CTX_LEN = 70;

export function encodeInitMatcherCtx(args: InitMatcherCtxArgs): Uint8Array {
  // kind must be 0 (Passive) or 1 (vAMM) — the wrapper validates this on-chain but
  // we reject early to surface misuse at the SDK layer.
  if (args.kind !== 0 && args.kind !== 1) {
    throw new Error(`encodeInitMatcherCtx: kind must be 0 (Passive) or 1 (vAMM), got ${args.kind}`);
  }
  // feeToInsuranceBps and skewSpreadMultBps are u16 — validate range.
  ...
  const data = concatBytes(
    encU8(IX_TAG.InitMatcherCtx),          // tag = 83 (u8)
    encU8(args.kind),                       // kind (u8)
    encU32(args.tradingFeeBps),             // trading_fee_bps (u32 LE)
    encU32(args.baseSpreadBps),             // base_spread_bps (u32 LE)
    encU32(args.maxTotalBps),               // max_total_bps (u32 LE)
    encU32(args.impactKBps),                // impact_k_bps (u32 LE)
    encU128(args.liquidityNotionalE6),      // liquidity_notional_e6 (u128 LE)
    encU128(args.maxFillAbs),               // max_fill_abs (u128 LE)
    encU128(args.maxInventoryAbs),          // max_inventory_abs (u128 LE)
    encU16(args.feeToInsuranceBps),         // fee_to_insurance_bps (u16 LE)
    encU16(args.skewSpreadMultBps),         // skew_spread_mult_bps (u16 LE)
  );
...
export function encodeAcceptAdmin(): Uint8Array {
  // Tag 82 is UnwrapEscrowedPortfolio in v17. The v12 AcceptAdmin wire format is incompatible.
