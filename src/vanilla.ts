...
// ─── Instruction tags ───────────────────────────────────────────────────────
// IX_TAG re-exported in full so callers can sanity-check tag bytes against
// any wrapper documentation. Listing only the vanilla subset would create a
// partial enum that confuses tooling.
export { IX_TAG, AUTHORITY_KIND } from "./abi/instructions.js";
export type { AuthorityKind, WrapperTarget } from "./abi/instructions.js";

// ─── Encoders (vanilla subset, alphabetical) ────────────────────────────────
export {
  encodeAcceptAdmin,
...
// ─── Argument types for the vanilla encoders ────────────────────────────────
export type {
  AdminForceCloseArgs,
  CloseAccountArgs,
  ...
  InitLPArgs,
  InitMarketArgs,
  InitMarketExtendedTail,
  InitUserArgs,
  KeeperCrankArgs,
  KeeperCrankCandidate,
  LiquidateAtOracleArgs,
  ReclaimEmptyAccountArgs,
...
// ─── Encoding primitives (so callers can build custom blobs) ────────────────
export {
  concatBytes,
  encU8,
  encU16,
